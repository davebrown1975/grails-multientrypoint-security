...
@Slf4j
class MontaguesAuthenticationProvider extends DaoAuthenticationProvider {
  ...
  Authentication authenticate(Authentication customAuth) {
    if (log.isDebugEnabled()) log.debug '{}', "Authenticating Montague"

    MontagueUser user = MontagueUser.findByUsername(customAuth.principal)
    if (user) {
